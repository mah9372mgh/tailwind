...
(function(window, document, $){
  ...
  window.v2Storage = {
    ...
    get: function(name, defaultValue) {
      ...
      var localStorageResult = window.localStorage.getItem(this.prefix + name);
      if (localStorageResult === null) {
        return defaultValue;
      }
      data = JSON.parse(localStorageResult);
      if ('t' in data && new Date(data.t) >= new Date()) {
        return data.v;
      }
      return defaultValue;
    },
    ...
    set: function(name, value, ttl) {
      ttl = typeof ttl !== 'undefined' ? ttl : 86400;
      var stDate = new Date();
      stDate.setTime(stDate.getTime() + ttl * 1000);
      window.localStorage.setItem(this.prefix + name, JSON.stringify({
        t: stDate.toString(),
        v: value
      }));
    }
   ...
   * @returns void
   */
  window.appendHit = function(model, id) {
   ...
   * @returns {String}
   */
   window.tra = function(key, args, options) {
  ...
  var i18nLoader = function() {
    var storageName = 'frontI18n:' + masterConfig.lang + ':' + masterConfig.v;
    var i18n = window.v2Storage.get(storageName , false);
    if (i18n === false) {
      $.ajax({
        ...
        success : function(json) {
          window.i18n = json;
          window.v2Storage.set(storageName, json, 604800);
        }
      });
    } else {
      window.i18n = i18n;
  ...
  if ('Clients' in window) {
      Clients.sync(function(number){},function(){});
  }

  if ('videojs' in window) {
      videojs.options.flash.swf = "/_v2/lib/misc/vidoe-js/video-js.swf";
  }
  ...
  if ('moment' in window) {
    ...
    var updateMoments = function() {
      $('*[data-moment]').each(function(){
        $(this).html(moment(new Date($(this).data('moment'))).fromNow());
      });
    };
 ...
 * @returns {GalleryData}
 */
function GalleryDataV2(id, successFunction, parameters) {
